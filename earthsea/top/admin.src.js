// Codice dell'AMMINISTRAZIONE di 'I Grandi di Terramare' (dalla 2.71).
//
...
function fxRidisegna(k){
  if (!FX_MARKUP[k]) return;
  renderList();
...
// ── Ⓘ 'Solo veri nomi noti' SEGUIVA le categorie accese (0.92): STORIA ───────────
// ⚠️⚠️ La casella è USCITA con la 2.10, e con lei `statoVeroNome`/`veroNomeInVigore`, che
// la forzavano con due casi estremi: soli ANIMALI la spegneva e la bloccava (nessuno dei 12
// ha un vero nome, quindi accesa dava lista vuota), soli DRAGHI la accendeva e la bloccava
// (tutti e 8 ce l'hanno, e per un drago il nome È il vero nome, quindi spegnerla non
// toglieva nulla). A sostituirla è la casella dei senza nome, per istruzione dell'utente:
// l'etichetta italiana è cambiata due volte, quindi qui non si nomina.
// ⚠️ La lezione che resta, ed è quella che pagava quel codice: una casella che in un certo
// stato può solo svuotare la lista, o che non può togliere niente, **non è un filtro** e
// mente su ciò che promette. Chi ne aggiunge una guardi i due estremi del dataset prima di
// darla per buona. ⚠️ La casella nuova non ricade nel caso: è additiva, quindi non svuota
// mai, e le voci che governa ci sono in tutte e tre le categorie.
// ⚠️ Il predicato `veroNomeNoto` NON è uscito con lei: regge il badge `veronoto` (vedi
// `haBadge`), che è il suo lavoro dalla 2.10.

// ── Ⓘ 'Personaggi senza nome' è stato un FLAG DI SITO dalla 1.07 alla 2.09 ───────
// ⚠️⚠️ DALLA 2.10 È UN FILTRO DEL PANNELLO (`mostraSenzaNome`, qui sopra), per istruzione
// dell'utente: la visibilità di quelle voci è passata da scelta editoriale del sito a
// preferenza di chi guarda. La storia serve a leggere i commit e le note vecchie, che lo
// dànno nella Console, e a non rimettercelo per riflesso.
// ⚠️⚠️ LA DISTINZIONE FRA I DUE POSTI RESTA VERA E VALE PER OGNI ALTRA VOCE: il **Pannello**
// è la modale del FAB, coi filtri del visitatore, aperta a tutti; la **Console** è l'editor
// admin, che salva in `dati.js` e vale per tutti i visitatori. La `1.06` mise questa voce nel
// primo e la `1.07` la spostò nel secondo, ed era la seconda volta che quella confusione
// costava una versione; dal 2026-08-28 l'editor si chiama **Console** proprio per questo,
// perché 'Pannello di controllo' conteneva l'altro nome e ogni abbreviazione li faceva
// collidere. ⚠️ Che stavolta la voce torni nel Pannello non smentisce niente di tutto questo:
// a cambiare è la natura della scelta, dichiarata dall'utente, non il criterio dei due posti.
...
// mentre un elenco qui dentro nessuno lo verrebbe a cercare.

// ── I mattoni della ricerca, comuni alla Console e al sito ──
...
var TYPE_LABEL = {
  // ⚠️ Il plurale del FILTRO resta al maschile perché indica la razza, non il singolo:
  // le categorie sono 'uomini', 'draghi' e 'animali'. Sulla singola card invece
  // l'etichetta segue il genere ('Uomo', 'Donna', 'Gatta'), perché in quel punto si parla
  // di un individuo.
  // Stesse etichette di `CAT_LABEL` (vedi là il perché di 'Persone'), con le chiavi
  // `type-*`: le due mappe vanno cambiate insieme. ⚠️ Il rinomino della 0.93 è arrivato
  // qui **per questa regola**, non per richiesta separata: l'utente aveva nominato il
  // Pannello, ma lasciare le Statistiche a dire 'Esseri umani' avrebbe fatto convivere due
  // parole per la stessa categoria nella stessa pagina, che è esattamente il difetto che
  // la nota qui sopra registra come già successo.
  // ⚠️⚠️ `type-donnadrago` sta QUI e non in `CAT_LABEL`, e non è una dimenticanza: NON è
  // una categoria di filtro (quelle sono due, umani e draghi), è una classe-etichetta che
  // le Statistiche contano a parte. Senza questa riga `typeName` ripiegava sulla classe
  // grezza e la tab 'Tipi' mostrava **'donnadrago'** in minuscolo, come l'utente ha visto
  // il 2026-08-23; la dicitura giusta è **'Ibridi'** (sua istruzione).
  // ⚠️ Il ripiego di `typeName` resta, ed è giusto che resti: mostra qualcosa invece di
  // niente. Ma è anche la ragione per cui il difetto non dà nessun errore, quindi una
  // classe-etichetta nuova nel dataset va aggiunta qui il giorno che nasce.
  it: { 'type-man':'Persone', 'type-dragon':'Draghi', 'type-donnadrago':'Ibridi', 'type-animale':'Animali' },
  en: { 'type-man':'People', 'type-dragon':'Dragons', 'type-donnadrago':'Hybrids', 'type-animale':'Animals' }
};
...
function showAdminEditor() {
  ...
  saveBtn.onclick = async () => {
    ...
    next.forEach((p, i) => {
      ...
      ICON_ORDER.forEach(function(k) {
        if (k === 'veronoto') return; // derivato da `vero_nome`: non ha checkbox e non ha campo (vedi `haBadge`)
  ...
  function buildCard(p, i) {
    ...
    ICON_ORDER.forEach(function(k) {
      // ⚠️⚠️ `veronoto` NON HA UNA CHECKBOX, e non è una dimenticanza: il badge si DERIVA da
      // `vero_nome` (vedi `haBadge`), che in questa stessa scheda è già un campo di testo
      // qui sopra. Una casella accanto a quel campo prometterebbe di poterlo contraddire, e
      // chi la spuntasse su una voce senza vero nome non vedrebbe cambiare niente.
      if (k === 'veronoto') return;
      ...
      iconEl.appendChild(svgNodo(BADGE_ICON[k].split('%L%').join('')));
  ...
  // ⚠️⚠️ `vero_nome` sta in TESTA e non è un dettaglio d'ordine: 17 voci hanno il campo
  // `nome` vuoto (Kalessin, Orm, Erreth-Akbe, Morred...), e per loro il vero nome è l'UNICO
  // nome che esista. Senza, `isName` le riconosceva ma il ciclo sui campi non trovava nulla
...
var SITE_FLAG_ITEMS = [
  ...
  // `noMob` come il riflettore, ma per la ragione della barra dinamica: su touch
  // l'alone è spento del tutto, quindi la sua variante mobile non muoverebbe nulla.
  { k:'vig', it:'Alone sfumato', en:'Soft halo', cfg:true, noMob:true },
  ...
  // ⚠️ `noMob` come la trama, e per una ragione affine: sotto i 480px la colonna
  // dell'origine non esiste, quindi la variante mobile non muoverebbe nulla.
  { k:'orig', it:'Origine', en:'Origin', cfg:true, noMob:true },
  // ⚠️ L'ordine delle due voci è stato INVERTITO nella 1.01 (richiesta dell'utente,
  // 2026-08-26): 'Origine' sta sopra 'Decorazione', dove prima era sotto. La posizione
  // di queste voci è una scelta dell'utente e non un ordine tecnico, quindi non si
  // 'sistema' a intuito. Senza `cfg`: è un interruttore e basta, come 'Dito che
  // scorre', perché la resa è già stata scelta fra tre mockup e qui serve solo
  // poterla spegnere. `noMob` perché la config è unica (vedi FX_UNI).
  { k:'deco', it:'Decorazione', en:'Corner mark', noMob:true },
  // ⚠️⚠️ QUI STAVA 'Senza nome', ED È USCITA CON LA 2.10 insieme al suo flag: la visibilità
  // delle voci senza nome d'uso è passata al Pannello, cioè da scelta del sito a filtro del
  // visitatore (istruzione dell'utente, 2026-09-14). ⚠️ Con lei la Console torna a governare
  // il SOLO aspetto, che era la sua natura prima della 1.07: chi cercasse qui l'interruttore
  // lo trovi nella riga dei filtri del Pannello, accanto ai due generi.
  // ⚠️ È l'unico effetto SOLO MOBILE, e la sua voce sta comunque nella tab Desktop: la
  // tab Mobile mostra le varianti `_m`, e questo ha config unica (`FX_UNI`) perché una
  // variante desktop non avrebbe nulla da governare. Il rovescio esatto di riflettore e
  // trama, che sono desktop-only e vivono lì per la stessa ragione strutturale.
  // ⚠️ ULTIMA voce, ed è l'ultima rimasta delle DUE che l'utente volle in fondo (1.00,
  // riconfermato il 2026-09-11: *le due voci mobile devono restare in fondo*): 'Tasti di
  // salto' se n'è andata con la 2.00, quando su mobile la colonna dei due tasti ha ceduto
  // il posto al glifo del FAB e quella manopola non ha avuto più niente da governare.
  // ⚠️ Il suo `gap` regge da solo dalla 2.10: fino a lì faceva coppia con quello di 'Senza
  // nome', e insieme segnavano l'isola della voce di contenuto. Adesso stacca la sola voce
  // mobile dal resto, che è la ragione per cui l'utente la volle in fondo.
  { k:'hint', it:'Dito che scorre', en:'Swipe hint', noMob:true, mobOnly:true, gap:true }
];
...
var FX_KNOBS = {
  ...
  pat: [
    ...
    { p:'mot', t:'sel', dep:'on', it:'Motivo', en:'Motif', opts:fxSelOpts('pat', 'mot'),
      nit:'I primi otto sono di Terramare: marea, correnti, risacca, mare, schiuma, sartiame, rose dei venti, maglia di scaglie. Gli altri tre vengono da Arda.',
      nen:'The first eight are of Earthsea: tide, currents, surf, sea, foam, rigging, wind roses, scale mail. The other three come from Arda.' },
  ...
  // ⚠️ L'origine non ha manopole numeriche: sono due scelte di RESA, e stanno qui
  // perché valgono per tutti i visitatori (vedi il default di `orig`).
  orig: [
    { p:'on', t:'bool', it:'Attiva', en:'Enable',
      nit:'La colonna con il luogo d\'origine, a destra della scheda. Sotto i 480px scende in linea sotto il contenuto, senza filetto.',
      nen:'The column with the place of origin, to the right of the card. Below 480px it drops in line under the content, with no rule.' },
    { p:'pin', t:'bool', dep:'on', it:'Segno di luogo', en:'Place mark',
      nit:'Il segnaposto della mappa al posto della parola \'origine\': non va tradotto e non prende posizione fra nascita e residenza.',
      nen:'The map pin instead of the word \'origin\': it needs no translation and takes no side between birth and residence.' },
    { p:'slot', t:'bool', dep:'on', it:'Spazio riservato', en:'Reserved space',
      nit:'Tiene la colonna anche sulle voci senza luogo della tabella principale, invece di lasciare che la scheda si allarghi. Costa una riga in piu\' a qualche citazione. Gli apocrifi la riservano sempre, e non dipendono da qui.',
      nen:'Keeps the column on the entries with no place in the main table, instead of letting the card widen. It costs one extra line to a few quotes. The apocryphal ones always reserve it, and do not depend on this.' }
  ],
};
...
// ⚠️⚠️ SEGNO 'SOLO MOBILE' accanto alle voci che governano cose che sul desktop non
// esistono (1.00, richiesta dell'utente). Serve perché quelle voci stanno nella tab
// **Desktop** pur riguardando il solo mobile: è una conseguenza della config unica
// (vedi FX_UNI), e senza un segno la contraddizione resta invisibile e si legge come un
// difetto. ⚠️ L'utente ha chiesto prima una dicitura e poi, meglio, un'icona: una parola
// in più in quella colonna avrebbe fatto andare a capo le etichette lunghe.
// ⚠️ Costruita a NODI e non con `innerHTML`, che è vietato: il markup accanto (`fx-gear`)
// lo usa ancora, ma quello è preesistente e non è una ragione per aggiungerne.
// ⚠️ `title` per il puntatore e `aria-label` sul nodo focalizzabile-non-focalizzabile: il
// solo `title` non arriva agli assistivi, e la sola `aria-label` non si vede col mouse.
function fxSegnoMobile(it){
  var NS = 'http://www.w3.org/2000/svg';
  var testo = it ? 'Solo sito mobile' : 'Mobile site only';
  var sp = document.createElement('span');
  sp.style.cssText = 'display:inline-flex;align-items:center;margin-left:0.4em;opacity:0.62;flex:none;';
  sp.title = testo;
  var svg = document.createElementNS(NS, 'svg');
  svg.setAttribute('viewBox', '0 0 12 16');
  svg.setAttribute('width', '10'); svg.setAttribute('height', '13');
  svg.setAttribute('role', 'img');
  svg.setAttribute('aria-label', testo);
  var corpo = document.createElementNS(NS, 'rect');
  corpo.setAttribute('x', '1.7'); corpo.setAttribute('y', '1');
  corpo.setAttribute('width', '8.6'); corpo.setAttribute('height', '14');
  corpo.setAttribute('rx', '1.8');
  corpo.setAttribute('fill', 'none'); corpo.setAttribute('stroke', 'currentColor');
  corpo.setAttribute('stroke-width', '1.2');
  var barra = document.createElementNS(NS, 'path');
  barra.setAttribute('d', 'M4.9 12.4h2.2');
  barra.setAttribute('stroke', 'currentColor'); barra.setAttribute('stroke-width', '1.2');
  barra.setAttribute('stroke-linecap', 'round');
  svg.appendChild(corpo); svg.appendChild(barra);
  sp.appendChild(svg);
  return sp;
}
...
// editor admin). I fondi sono quelli REALI del sito (#0d1a22 / #f9fbfa) e la card
...
function injectFxEditorCss(){
  ...
  st.textContent = [
    ...
    '.fxp-pane.d{background:#0d1a22;color:#d2d2d2}',
    '.fxp-pane.l{background:#f9fbfa;color:#373737}',
  ...
  knobs.filter(function(kn){ return !kn.th || kn.th === kTheme; }).forEach(function(kn){
    ...
    if (kn.t === 'bool') {
      ...
      cb.addEventListener('change', function(){
        ...
        applySiteFlags(); paint(); renderKnobs(); fxRidisegna(key);
      });
      row.appendChild(cb);
    } else if (kn.t === 'sel') {
...
function showSiteFlagsEditor(initState){
  ...
  // ⚠️ `fxRidisegna('orig')` perché l'origine è markup: senza, il ripristino tornerebbe
  // vero nei flag ma non in pagina (stessa ragione per cui la casella la richiama).
  var annulla = function(){ SITE_FLAGS = normSiteFlags(SITE_FLAGS_SAVED); applySiteFlags(); reflowRows(); fxRidisegna('orig'); close(); };
  ...
  function renderRows(){
    ...
    SITE_FLAG_ITEMS.forEach(function(f){
      // Col flag `xlMode` spento la riga della XL non si mostra affatto: un
      // interruttore che non commuta niente è peggio della sua assenza, perché
      // sembra rotto invece di dichiarare che la funzione non c'è.
      if (f.k === 'zoomBig' && !FEATURES.xlMode) return;
      if ((f.k === 'zoomBig' || f.plain || f.noMob) && tab === 'm') return; // XL, i flag piatti e gli effetti `noMob` (FX_UNI): solo nella tab Desktop
      ...
      var vk = (f.k === 'zoomBig' || f.plain) ? f.k : f.k + sfx;
      var row = document.createElement('div');
      row.style.cssText = 'display:grid;grid-template-columns:auto 1fr auto;gap:0.55rem;align-items:center;margin:'
        + (f.gap ? '0.5rem' : '0') + ' 0 0.62rem;';
      var cb = document.createElement('input'); cb.type = 'checkbox';
      ...
      cb.addEventListener('change', function(){
        ...
        // ⚠️⚠️ La CASELLA di un effetto che tocca il markup deve RIDISEGNARE (richiesta
        // dell'utente, 2026-08-26: la spunta di 'Origine' deve agire come mostra/nascondi,
        // con effetto immediato). Il rimedio esisteva già (`fxRidisegna`, 0.91) ma era
        // agganciato alle sole manopole della sotto-modale: da qui l'interruttore cambiava
        // il flag e basta, e la colonna dell'origine restava in pagina con le sue
        // impostazioni. `applySiteFlags` non basta perché quella colonna è markup scritto
        // da `renderList`, non una classe sul documento.
        fxRidisegna(f.k);
      ...
      if (f.mobOnly) txt.appendChild(fxSegnoMobile(it));
  ...
  function syncVarWarn(){
    ...
    if (!isMob) SITE_FLAG_ITEMS.forEach(function(f){
      if (f.k === 'zoomBig' || f.plain || f.noMob) return;
      tutte.push(f);
  ...
  // Col flag spento non c'è nessuna precedenza da spiegare: la preferenza
  // personale può esistere ancora nel `localStorage` (non la cancelliamo, così
  // riaccendendo il flag l'utente la ritrova) ma non ha più effetto, e mostrare
  // l'avviso direbbe il falso.
  var mine = FEATURES.xlMode ? LS.getItem(ZOOM_LS_KEY) : null;
  if (mine !== null) {
...
function baInjectEditorCss(){
  ...
  var css = [
    ...
    'html[data-theme="light"] .ba-fval{color:#2e4145;}',
    ...
    'html[data-theme="light"] .ba-fline input[type=range]::-webkit-slider-thumb{background:#2e4145;}',
    'html[data-theme="light"] .ba-fline input[type=range]::-moz-range-thumb{background:#2e4145;}',
...
function showBadgeAdjustEditor(initState){
  ...
  baInjectEditorCss();
  var it = currentLang === 'it';
  // ⚠️ L'unità di partenza si legge dall'ELENCO, non da un id scritto a mano: qui c'era
  // `'helcaraxe'`, cioè un'unità di Arda, e su Terramare apriva l'editor su un id
  // inesistente. Effetto: `BADGE_ADJUST[cur]` undefined e la modale che moriva con un
  // TypeError in `showVal`, quindi l'editor non si apriva affatto.
  var cur = (initState && initState.cur) || BADGE_ADJUST_UNITS[0].id;
  var myL;
  ...
  // ⚠️ Le classi `si-pal-*` portano la palette del riquadro, che non è quella del tema in
  // corso: i due fondi convivono nella stessa pagina, e senza di esse le icone dell'uno e
  // dell'altro rendevano tutte le tinte del documento. I valori vivono nel CSS delle due
  // palette, in testa al file, e qui non si ripetono.
  var paneD = document.createElement('div'); paneD.className = 'ba-pane dark si-pal-scuro';
  var paneL = document.createElement('div'); paneL.className = 'ba-pane light si-pal-chiaro';
  ...
  function samples(u){
    ...
    for (i = 0; i < dati.length && out.length < 3; i++){ p = dati[i]; if (u.members.some(function(k){ return haBadge(p, k); })) out.push(p); }
    return out;
  }
  // ⚠️ Dalla 2.15 le icone sono SVG IN LINEA, quindi non esiste più nessun `src` da cui
  // ricavare un percorso: l'anteprima inietta lo stile sull'`<svg>`, che è la stessa via
...
function showColorStats(initState) {
  ...
  function statRow(host, o){
    ...
    // ⚠️ Il conteggio si compone a NODI: `innerHTML` è vietato senza eccezioni (regola non
    // derogabile), anche quando il contenuto è un numero calcolato da noi.
    var cnt = document.createElement('span'); cnt.style.cssText = 'font-variant-numeric:tabular-nums;text-align:right;opacity:0.85;min-width:56px;';
    var cntB = document.createElement('b'); cntB.textContent = String(o.n); cnt.appendChild(cntB);
    cnt.appendChild(document.createTextNode(' · ' + pct + '%')); row.appendChild(cnt);
    if (o.onClick){
  ...
  var stack = [];
  function renderTop(){ content.textContent = ''; box.scrollTop = 0; stack[stack.length - 1](content); }
  ...
  function charList(host, chars){
    ...
    chars.forEach(function(p){
      ...
      r.textContent = nomeDiRif(p, it); wrap.appendChild(r);
    });
  ...
  function catDetailView(catKey, catName){
    return function(host){
      ...
      // ⚠️ `categorie` (plurale) e non `categoria`: un'IBRIDA appartiene a DUE categorie, e
      // aprendo 'Draghi' deve comparire anche lei. Col singolare (la prima categoria del
      // tipo) Therru entrava nel dettaglio degli umani e spariva da quello dei draghi,
      // mentre il filtro del Pannello la mostrava in entrambi: due punti della stessa
      // pagina che dicevano cose diverse.
      var counts = {}; dati.forEach(function(p){ if (categorie(p).indexOf(catKey) === -1) return; typeClassesOf(p).forEach(function(c){ if (c) counts[c] = (counts[c] || 0) + 1; }); });
  ...
  function catView(host){
    // ⚠️⚠️ Si conta con `categorie` (plurale), quindi un'IBRIDA vale in DUE righe: appartiene
    // a entrambe le categorie (istruzione dell'utente, 2026-08-23: *a livello di categoria
    // devono appartenere sia a umani che a draghi... appariranno sia con solo Draghi attivo
    // che con solo Umani attivo*), ed è già così nel filtro del Pannello. Col singolare qui
    // i draghi erano SEI e nel filtro SETTE, cioè la stessa pagina si contraddiceva.
    // ⚠️ Conseguenza da dichiarare, non da nascondere: la somma delle righe può superare il
    // totale dei personaggi, e per questo la riga di sintesi lo dice quando succede. È lo
    // stesso patto della tab 'Tipi', che parla di etichette e non di persone.
    var counts = {}; dati.forEach(function(x){ categorie(x).forEach(function(c){ counts[c] = (counts[c] || 0) + 1; }); });
    ...
    var doppie = dati.filter(function(x){ return categorie(x).length > 1; }).length;
    ...
    subLine(host, total + (it?' personaggi in ':' characters in ') + rows.length + (it?' categorie':' categories')
      + (doppie ? (it ? ' (' + doppie + (doppie === 1 ? ' ibrido conta' : ' ibridi contano') + ' in due)'
                      : ' (' + doppie + (doppie === 1 ? ' hybrid counts' : ' hybrids count') + ' in two)') : ''));
  ...
  // ⚠️⚠️ ANTI-JITTER VERTICALE FRA LE TRE SCHEDE (0.91, segnalazione dell'utente: *le tab
  // Famiglie e Tipi hanno la stessa altezza, ma Categorie è più bassa e la modale balla in
  // modo fastidioso al passaggio*). Famiglie ha quattro righe e Categorie tre, quindi la
  // modale si accorciava e si riallungava a ogni giro di schede.
  // ⚠️ L'altezza NON si indovina e non si scrive a numero: si MISURA disegnando le tre
  // viste una dopo l'altra e tenendo la maggiore, come per le gemelle bilingui delle card.
  // Un `min-height` a occhio sarebbe sbagliato al primo cambio di dataset, e qui il numero
  // delle righe viene dai dati (le famiglie sono quattro oggi).
  // ⚠️ Riserva le sole viste BASE: le drill-down (l'elenco dei personaggi di una famiglia)
  // sono un altro livello e possono crescere quanto vogliono, tanto ci si entra apposta.
  // ⚠️ Due `requestAnimationFrame`: il primo consegna il frame in cui la modale entra nel
  // DOM, il secondo quello in cui ha già la sua larghezza, e le righe si misurano solo
  // allora. Ⓘ Il limite dichiarato: la riserva è calcolata all'apertura, quindi un resize a
  // modale aperta non la ricalcola; il cambio lingua o tema sì, perché ricostruisce tutto.
  requestAnimationFrame(function(){ requestAnimationFrame(function(){
    if (!document.body.contains(ov)) return;
    var attiva = curTab, alta = 0;
    ['fam', 'cat', 'type'].forEach(function(k){
      stack = [ k === 'fam' ? famView : k === 'cat' ? catView : typeView ];
      renderTop();
      alta = Math.max(alta, content.offsetHeight);
    });
    setStatTab(attiva);
    if (alta) content.style.minHeight = alta + 'px';
  }); });
  if (initState && initState.scroll) box.scrollTop = initState.scroll;
...
function showColorEditor(initState) {
  ...
  function buildMirata(root){
    ...
    function renderResults(q){
      ...
      for (var i = 0; i < dati.length && hits.length < 20; i++){
        var p = dati[i];
        if (nomeDiRif(p, true).toLowerCase().indexOf(q) !== -1 || nomeDiRif(p, false).toLowerCase().indexOf(q) !== -1) hits.push(i);
      }
      hits.forEach(function(i){
        ...
        row.textContent = nomeDiRif(p, it) + '  ·  ' + familyOf(p);
  ...
  function selectChar(i, panel){
    ...
    name.textContent = nomeDiRif(p, it);
    ...
    var bSave = mkBtn(it ? 'Salva sul repo' : 'Save to repo', function(btn){ apply(); withBtn(btn, saveColorsToRepo('colori: colore individuale di ' + nomeDiRif(p, true))); }, true);
  ...
  function renderPreview(host, darkHex, lightHex, sampleName, sampleTipo){
    ...
    [['light', lightHex, '#f9fbfa', '#eff3f2', 0.05, 'rgba(30,30,34,0.92)'],
     ['dark', darkHex, '#0d1a22', '#071016', 0.10, 'rgba(232,232,232,0.92)']].forEach(function(row){
