...
- ⚠️⚠️ **`jumpx` NON ESISTE PIÙ, DALLA `2.00`**: era l'ultima voce, con una manopola numerica
  in px, e regolava lo scostamento dei due tasti dal bordo destro **solo su mobile**. Con la
  colonna che su mobile ha ceduto il posto al glifo del FAB non governava più niente, e una
  manopola che non fa niente è codice morto. Con lei sono usciti la classe di anteprima, la
  regola iniettata e l'intera `fxAnteprima`.
  - Ⓘ **Quello che la sua storia insegna resta, perché il caso può ricapitare**: c'erano DUE
    cose non ovvie, e valgono per qualunque manopola futura nella stessa condizione.
    1. **Il valore statico nel CSS era lo stato SPENTO, non il default**, e la differenza non è
       formale: col default là, spegnere la manopola non riportava i tasti a filo e la nota
       della manopola diceva il falso. Misurato prima del rimedio: a manopola spenta restavano
       a 2,33px.
    2. **L'anteprima in pagina non era codice dedicato**: la regola la iniettava
       `injectFxRules`, che gli slider richiamano a ogni `input`, quindi il valore arrivava in
       pagina mentre si trascinava. L'unica riga sua era la classe che portava i tasti **a
       galla**, perché si dissolvono da soli e vivono sotto il velo della modale, opaco al 92%.
       ⚠️ E il suo timer si **riarmava** a ogni movimento invece di contare dal primo:
       trascinando per più di un secondo, un timer che parte una volta sola fa sparire
       l'anteprima proprio mentre la si usa.
...
- ⚠️ **Il segno 'solo mobile'** (una piccola icona di smartphone, `title` e `aria-label`
  *Solo sito mobile*) accanto a 'Dito che scorre': quella voce è nella tab **Desktop** pur
  riguardando il solo mobile, che è una conseguenza della config unica (`FX_UNI`), e senza un
  segno la contraddizione si legge come un difetto. **Una dicitura a parole è la strada
  scartata** (prima idea dell'utente, corretta da lui stesso): in quella colonna avrebbe
  mandato a capo le etichette lunghe.
  - Ⓘ **Erano due, e la seconda era 'Tasti ⤒⤓'**, uscita con `jumpx` nella `2.00`. I suoi
    glifi erano stati **verificati nel font**, non dati per scontati, e il metodo resta buono
    per il prossimo: un glifo mancante non dà errore, disegna un rettangolo vuoto, quindi si
    confronta la larghezza del testo con quella di un codepoint sicuramente assente (area a
    uso privato). Là: 13,02px contro 22,91px, quindi il glifo era vero.

#### ↔️ Ⓘ I tasti di salto su mobile: STORIA, e la lezione sulle unità

⚠️ **Dalla `2.00` su mobile quella colonna non c'è più**, quindi qui non resta niente da
regolare. La voce sopravvive per la **lezione**, che non riguarda i tasti e ricapiterà.

L'utente li spostò a sinistra in **due riprese**, prima ~5px e poi altri 2.
⚠️⚠️ **Erano device px a DPR 3, quindi 7 in tutto valevano 2,33px CSS** (`0.1458rem`):
prenderli per buoni avrebbe spostato le freccine del **triplo**. La regola è universale
(`Roccobot.md` § '🎨 Grafica'), e il caso concreto aggiunge una cosa che la regola non dice:
la **seconda** richiesta diceva solo *altri 2px*, senza ripetere il DPR, ed è stata letta con
la stessa unità della prima **perché era la stessa misura sullo stesso screenshot**. Verificato
dal DOM: 2,328px CSS, cioè 6,98 device px.
...
### ↕️ Ⓘ Le freccine a FILO del bordo: STORIA, e la misura che resta

⚠️ **Dalla `2.00` su mobile quella colonna non c'è più** (vedi § 'Il salto in cima e in fondo,
sul glifo del FAB'), quindi questa voce non descrive più niente di vivo: resta perché la
**misura scartata** è quella che aveva deciso, e chi un domani rimettesse dei tasti flottanti
riparta da lì invece che da zero.

Dalla `0.71` alla `2.00` il rientro mobile era il solo `env(safe-area-inset-right)`, cioè a
filo del bordo, dopo una segnalazione dell'utente: cadevano sopra la linea destra del riquadro
della citazione. Col tondo spento su mobile (`FEATURES.jumpMobileCircle`) si vedeva la sola
freccia, **16px** dentro un box di **26,4**, e il riquadro della citazione finiva a **23,4px**
dal bordo dello schermo, cioè **meno** della larghezza del box: nessun rientro poteva togliere
la sovrapposizione al box, e per questo si ragionava sulla freccia. A 390px: `0.75rem` la
metteva **9,8px dentro** la linea, `0.2rem` ancora 1px dentro, a filo del bordo stava **2,2px
fuori** restando a 5,2px dallo schermo.

## ⏫ Il salto in cima e in fondo, sul glifo del FAB

⚠️⚠️ **DALLA `2.00` I DUE TASTI SU MOBILE NON CI SONO PIÙ: A PORTARE IN CIMA E IN FONDO È IL
...
della doppia lingua*), che è giusta e vale per una manciata di card; poi la via d'uscita (*non si
può provare a impostare 3 misure diverse? Con discendenti, senza e misto*), che quelle le mette
in una classe loro invece di lasciarle rompere le altre.

⚠️⚠️ **E DALLA `2.59` I VALORI SONO ALTRI E IL CRITERIO È UN ALTRO ANCORA**, per una seconda
istruzione dell'utente dello stesso giorno: *sceglierò io tre valori... lo scopo finale è
spaziare un po' di più il caso senza discendenti e un po' di meno quello con, in modo che si
...
predicato guardava la stringa intera e le dava 9px di aria per una lettera che sta altrove.

- ⚠️ **Perciò la decisione dipende dal LAYOUT e non dal solo testo**, e si rifà a ogni reflow
  insieme a `freeNames`, che quel giro di misure lo fa già. La riga della `2.58` che la dava
...
- ⚠️⚠️ **I DUE METRI NON SONO INTERCAMBIABILI, e servono tutti e due**: la `x` di un carattere si
  ...
  nulla. Chiedere l'una all'altro dà una misura che sembra giusta e non lo è.
  - ⚠️ **La soglia dei 2px separa il discendente dall'antialiasing**: in EB Garamond 700 `g j q y`
    scendono di 9px su un corpo di 29,12 e `p Q ç` di 8, mentre `d x M` danno 1px. Regge anche
    nelle fasce strette, dove il corpo scende a ~19px e quel px diventa 0,7.
  - ⚠️ **Il font caricato non conta**: se EB Garamond non è ancora pronto il canvas misura il
    serif di ripiego, che ha i discendenti sulle **stesse** lettere, e dalla misura non passa
    nessun pixel, perché il margine è in em nel CSS.
- **Costa 0,56ms a reflow**, contro i 41 di `freeNames` e gli 81 di `reflowRows`: meno dell'1%,
  quindi il ricalcolo a ogni giro non è un prezzo da discutere.
- **Misure del giro**: **53 controlli su 53** su dodici larghezze da 1440 a 320, coi font veri.
  Zero card cambiano altezza, posizione o **classe** al cambio lingua, le card con le icone a capo
  tengono il loro valore, e `freeNames` libera le stesse card di prima.
  - ⚠️⚠️ **IL METRO DELLO STACCO SI PRENDE DALLA PUNTA DEL DISCENDENTE, NON DALLA BASELINE**, ed è
    l'errore che questo banco ha fatto al primo giro: misurando dalla baseline il caso col
    discendente risulta di 8-9px più largo di quello che l'occhio vede, quindi `Pioppo` dava 24,66
    invece di 16,66 e la prova accusava un codice giusto. Lo scarto è esattamente il discendente,
    ed è la spia che riconosce il caso.
  - ⚠️ **La baseline va costruita, perché nessuna proprietà CSS la dà**: si ricava dal rettangolo
    di riga del `Range` più le metriche del canvas (`fontBoundingBoxAscent/Descent` per centrarla
    nell'interlinea, `actualBoundingBox*` per l'estensione vera), e sul vero nome il testo va
    messo in **maiuscolo** prima di misurarlo, o il canvas misura le minuscole del dato mentre la
    pagina rende le maiuscole.
